...
class TitleBar : ViewGroup, View.OnClickListener {
    ...
    fun setTitleImageResource(resId: Int) {
        mCenterText!!.setCompoundDrawablesWithIntrinsicBounds(resId, 0, 0, 0)
    }
    ...
    private fun inflateAction(action: Action): View {
        val view: View?
        when {
            action.customView != null -> view = action.customView
            TextUtils.isEmpty(action.text) -> {
                val img = ImageView(context)
                if (null != action.drawable) {
    ...
    fun getViewByAction(action: Action): View = findViewWithTag(action)

    override fun onMeasure(widthMeasureSpec: Int, heightMeasureSpec: Int) {
        var _heightMeasureSpec = heightMeasureSpec
        val heightMode = View.MeasureSpec.getMode(_heightMeasureSpec)
        val height: Int
        if (heightMode != View.MeasureSpec.EXACTLY) {
            height = mHeight + mStatusBarHeight
            _heightMeasureSpec = View.MeasureSpec.makeMeasureSpec(mHeight, View.MeasureSpec.EXACTLY)
        } else {
            height = View.MeasureSpec.getSize(_heightMeasureSpec) + mStatusBarHeight
        }

        measureChild(mLeftText, widthMeasureSpec, _heightMeasureSpec)
        measureChild(mRightLayout, widthMeasureSpec, _heightMeasureSpec)
        if (mLeftText!!.measuredWidth > mRightLayout!!.measuredWidth) {
            mCenterLayout!!.measure(
                    View.MeasureSpec.makeMeasureSpec(mScreenWidth - 2 * mLeftText!!.measuredWidth, View.MeasureSpec.EXACTLY), _heightMeasureSpec)
        } else {
            mCenterLayout!!.measure(
                    View.MeasureSpec.makeMeasureSpec(mScreenWidth - 2 * mRightLayout!!.measuredWidth, View.MeasureSpec.EXACTLY), _heightMeasureSpec)
        }
        measureChild(mDividerView, widthMeasureSpec, _heightMeasureSpec)
    ...
    abstract class ImageAction(@param:DrawableRes override val drawable: Int) : Action {

        override val text: String?
            get() = null

        override val customView: View?
            get() = null
    }
    ...
    abstract class CustomAction(override val customView: View) : Action {
    ...
    companion object {
        private val DEFAULT_MAIN_TEXT_SIZE = 20
        private val DEFAULT_SUB_TEXT_SIZE = 12
        private val DEFAULT_ACTION_TEXT_SIZE = 16
